...
final class TestKitCoverageTests: XCTestCase {
  ...
  func testTranscriptReplayCoversAllOperations() async throws {
    let entries: [TranscriptEntry] = [
      ...
      TranscriptEntry(
        operation: "executeCommand", response: TranscriptData(code: 0x2001, params: [1, 2])),
      TranscriptEntry(
        operation: "executeStreamingCommand",
        response: TranscriptData(code: 0x2001, params: [3, 4])),
    ...
    let infosByFilter = try await replay.getObjectInfos(
      storage: MTPStorageID(raw: 1), parent: 10, format: 0x3001)
    XCTAssertEqual(infosByFilter.count, 1)
    XCTAssertEqual(infosByFilter[0].parent, 10)

    let execute = try await replay.executeCommand(
      PTPContainer(type: 1, code: 0x1001, txid: 3, params: []))
  ...
  func testVirtualMTPDeviceMutationReadWriteAndEvents() async throws {
    ...
    let createdFolder = try await device.createFolder(
      parent: nil, name: "Created", storage: storageId)
