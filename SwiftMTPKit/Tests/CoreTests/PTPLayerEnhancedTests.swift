...
final class PTPLayerEnhancedTests: XCTestCase {
    ...
    func testGetStorageIDsWithStorages() async throws {
        ...
        let deviceId = MTPDeviceID(raw: "test:001@1:1")
        let summary = MTPDeviceSummary(id: deviceId, manufacturer: "Test", model: "Test", vendorID: 0x1234, productID: 0x5678)
        let info = MTPDeviceInfo(manufacturer: "Test", model: "Test", version: "1.0", serialNumber: nil, operationsSupported: Set([0x1001, 0x1002, 0x1004]), eventsSupported: [])
        let config = VirtualDeviceConfig(deviceId: deviceId, summary: summary, info: info, storages: [storage1, storage2])
        let link = VirtualMTPLink(config: config)
    ...
    func testGetStorageInfoSuccess() async throws {
        ...
        let deviceId = MTPDeviceID(raw: "test:001@1:1")
        let summary = MTPDeviceSummary(id: deviceId, manufacturer: "Test", model: "Test", vendorID: 0x1234, productID: 0x5678)
        let info = MTPDeviceInfo(manufacturer: "Test", model: "Test", version: "1.0", serialNumber: nil, operationsSupported: Set([0x1001, 0x1005]), eventsSupported: [])
        let config = VirtualDeviceConfig(deviceId: deviceId, summary: summary, info: info, storages: [storage])
        let link = VirtualMTPLink(config: config)
    ...
    func testGetObjectHandlesWithObjects() async throws {
        let obj1 = VirtualObjectConfig(handle: 0x00010001, storage: MTPStorageID(raw: 0x00010001), parent: nil, name: "file1.txt")
        let obj2 = VirtualObjectConfig(handle: 0x00010002, storage: MTPStorageID(raw: 0x00010001), parent: nil, name: "file2.txt")
        let storage = VirtualStorageConfig(id: MTPStorageID(raw: 0x00010001), description: "Internal", capacityBytes: 16_000_000_000)
        
        let deviceId = MTPDeviceID(raw: "test:001@1:1")
        let summary = MTPDeviceSummary(id: deviceId, manufacturer: "Test", model: "Test", vendorID: 0x1234, productID: 0x5678)
        let info = MTPDeviceInfo(manufacturer: "Test", model: "Test", version: "1.0", serialNumber: nil, operationsSupported: Set([0x1001, 0x1007]), eventsSupported: [])
        let config = VirtualDeviceConfig(deviceId: deviceId, summary: summary, info: info, storages: [storage], objects: [obj1, obj2])
        let link = VirtualMTPLink(config: config)
    ...
    func testGetObjectHandlesWithParentFilter() async throws {
        let parentObj = VirtualObjectConfig(handle: 0x00000001, storage: MTPStorageID(raw: 0x00010001), parent: nil, name: "folder", formatCode: 0x3001)
        let childObj = VirtualObjectConfig(handle: 0x00010002, storage: MTPStorageID(raw: 0x00010001), parent: 0x00000001, name: "child.txt")
        let storage = VirtualStorageConfig(id: MTPStorageID(raw: 0x00010001), description: "Internal", capacityBytes: 16_000_000_000)
        
        let deviceId = MTPDeviceID(raw: "test:001@1:1")
        let summary = MTPDeviceSummary(id: deviceId, manufacturer: "Test", model: "Test", vendorID: 0x1234, productID: 0x5678)
        let info = MTPDeviceInfo(manufacturer: "Test", model: "Test", version: "1.0", serialNumber: nil, operationsSupported: Set([0x1001, 0x1007]), eventsSupported: [])
        let config = VirtualDeviceConfig(deviceId: deviceId, summary: summary, info: info, storages: [storage], objects: [parentObj, childObj])
        let link = VirtualMTPLink(config: config)
        
        let storageID = MTPStorageID(raw: 0x00010001)
        let handles = try await PTPLayer.getObjectHandles(storage: storageID, parent: 0x00000001, on: link)
    ...
    func testSupportsOperationWithAllOperations() {
        ...
        XCTAssertTrue(PTPLayer.supportsOperation(PTPOp.getObjectHandles.rawValue, deviceInfo: deviceInfo))
    ...
    func testFullSessionLifecycle() async throws {
        ...
        let deviceId = MTPDeviceID(raw: "test:001@1:1")
        let summary = MTPDeviceSummary(id: deviceId, manufacturer: "TestManufacturer", model: "TestModel", vendorID: 0x1234, productID: 0x5678)
        let info = MTPDeviceInfo(
            manufacturer: "TestManufacturer",
            model: "TestModel",
            version: "1.0",
