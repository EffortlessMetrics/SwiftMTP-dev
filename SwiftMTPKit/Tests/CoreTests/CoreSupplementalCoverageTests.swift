...
final class CoreSupplementalCoverageTests: XCTestCase {
    ...
    func testMTPEventFromRawParsesKnownCodes() {
        if case .objectAdded(let handle)? = MTPEvent.fromRaw(makeEventContainer(code: 0x4002, param: 777)) {
            XCTAssertEqual(handle, 777)
        } else {
            XCTFail("Expected objectAdded event")
        }

        if case .objectRemoved(let handle)? = MTPEvent.fromRaw(makeEventContainer(code: 0x4003, param: 888)) {
            XCTAssertEqual(handle, 888)
        } else {
            XCTFail("Expected objectRemoved event")
        }

        if case .storageInfoChanged(let storage)? = MTPEvent.fromRaw(makeEventContainer(code: 0x400C, param: 0x10001)) {
