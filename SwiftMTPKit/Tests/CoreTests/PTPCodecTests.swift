...
final class PTPCodecTests: XCTestCase {
  ...
  func testObjectInfoDatasetBasic() {
    ...
    // Verify first fields
    var offset = 0
    let storageID = data.withUnsafeBytes { $0.load(fromByteOffset: offset, as: UInt32.self) }
      .littleEndian
    XCTAssertEqual(storageID, 0x00010001)
    offset += 4

    let format = data.withUnsafeBytes { $0.load(fromByteOffset: offset, as: UInt16.self) }
      .littleEndian
