...
final class FallbackLadderTests: XCTestCase {
  ...
  func testAllFailedThrowsFallbackAllFailedError() async throws {
    ...
    do {
      ...
      XCTAssertNotNil(e.attempts[0].error)
      XCTAssertNotNil(e.attempts[1].error)
    } catch {
      XCTFail("Expected FallbackAllFailedError, got \(error)")
    }
  }
  ...
  func testMTPErrorNotSupportedFallback() async throws {
    ...
    do {
      ...
      // The inner MTPError is recorded in the attempt
      XCTAssertEqual(e.attempts.count, 1)
      XCTAssertFalse(e.attempts[0].succeeded)
      let errText = e.attempts[0].error ?? ""
      XCTAssertTrue(errText.contains("notSupported"), "Expected notSupported in: \(errText)")
    } catch {
      XCTFail("Expected FallbackAllFailedError, got \(error)")
    }
  }
