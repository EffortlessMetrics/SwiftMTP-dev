...
@Suite(.tags(.performance, .regression))
struct PerformanceRegressionTests {
  ...
  @Test("Throughput benchmark for file transfers")
  func testThroughputBenchmark() async throws {
    ...
    // Record result
    let throughputMbps = result.bytesTransferred / result.durationMs / 1000.0
    
    // Compare against baseline
    if let baseline = Self.baselineStorage.getBaseline(for: "throughput-100m") {
      #expect(throughputMbps >= baseline * 0.9) // Allow 10% regression
    } else {
      // Store baseline for first run
      Self.baselineStorage.setBaseline(throughputMbps, for: "throughput-100m")
    }
  }
  
  @Test("Small file transfer latency benchmark")
  func testSmallFileLatency() async throws {
    ...
    #expect(avgLatency < 100) // Should be under 100ms average
  }
  
  @Test("Large file sequential transfer")
  func testLargeFileSequentialTransfer() async throws {
    ...
    for size in sizes {
      ...
      #expect(throughput > 0) // Must have positive throughput
    }
  }
  ...
  func testMemoryUnderLoad() async throws {
    ...
    // Get peak memory
    let peakMemory = memoryTracker.peakMemoryUsage()
    
    #expect(peakMemory < 500 * 1024 * 1024) // Under 500MB peak
  }
  ...
  func testMemoryLeakDetection() async throws {
    ...
    // Baseline memory
    let baselineMemory = leakDetector.currentMemory()
    ...
    #if canImport(Darwin)
    // On Darwin, we can't force GC but we can check trends
    #endif
    
    // Check for significant memory growth
    let finalMemory = leakDetector.currentMemory()
    let growth = finalMemory - baselineMemory
    
    #expect(growth < 10 * 1024 * 1024) // Less than 10MB growth
  }
  
  // MARK: - CI Reporting Tests
  
  @Test("Performance metrics output for CI")
  func testPerformanceMetricsOutput() async throws {
    ...
    #expect(report.contains("Latency"))
  ...
  func testBaselineComparison() async throws {
    ...
    #expect(comparison.regressions.count >= 0) // May have regressions
    #expect(comparison.improvements.count >= 0) // May have improvements
  }
}
