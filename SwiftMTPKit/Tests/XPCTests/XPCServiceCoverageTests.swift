...
final class XPCServiceCoverageTests: XCTestCase {
    private func configuredService() async -> (impl: MTPXPCServiceImpl, stableId: String, ephemeralId: MTPDeviceID, storageId: UInt32, objectHandle: UInt32) {
    ...
    private func storages(_ impl: MTPXPCServiceImpl, request: StorageListRequest) async -> StorageListResponse {
        await withCheckedContinuation { continuation in
    ...
    private func objects(_ impl: MTPXPCServiceImpl, request: ObjectListRequest) async -> ObjectListResponse {
        await withCheckedContinuation { continuation in
    ...
    private func objectInfo(_ impl: MTPXPCServiceImpl, deviceId: String, storageId: UInt32, objectHandle: UInt32) async -> ReadResponse {
        await withCheckedContinuation { continuation in
            impl.getObjectInfo(deviceId: deviceId, storageId: storageId, objectHandle: objectHandle) { response in
                continuation.resume(returning: response)
            }
        }
    }

    private func crawl(_ impl: MTPXPCServiceImpl, request: CrawlTriggerRequest) async -> CrawlTriggerResponse {
        await withCheckedContinuation { continuation in
    ...
    private func status(_ impl: MTPXPCServiceImpl, request: DeviceStatusRequest) async -> DeviceStatusResponse {
        await withCheckedContinuation { continuation in
    ...
    func testServiceImplSuccessAndErrorPaths() async throws {
        ...
        let missingRead = await read(impl, request: ReadRequest(deviceId: "missing-device", objectHandle: config.objectHandle))
        XCTAssertFalse(missingRead.success)
        XCTAssertNotNil(missingRead.errorMessage)

        let successfulRead = await read(impl, request: ReadRequest(deviceId: config.stableId, objectHandle: config.objectHandle))
        ...
        let missingStorages = await storages(impl, request: StorageListRequest(deviceId: "missing-device"))
        XCTAssertFalse(missingStorages.success)
        XCTAssertNotNil(missingStorages.errorMessage)

        let storageResponse = await storages(impl, request: StorageListRequest(deviceId: config.stableId))
        XCTAssertTrue(storageResponse.success)
        XCTAssertTrue((storageResponse.storages ?? []).contains(where: { $0.storageId == config.storageId }))

        let objectResponse = await objects(
            impl,
            request: ObjectListRequest(deviceId: config.stableId, storageId: config.storageId, parentHandle: nil)
        )
        XCTAssertTrue(objectResponse.success)
        XCTAssertTrue((objectResponse.objects ?? []).contains(where: { $0.handle == config.objectHandle }))
    ...
    func testServiceImplCrawlStatusAndCleanupBranches() async throws {
        ...
        let defaultCrawl = await crawl(
            impl,
            request: CrawlTriggerRequest(deviceId: config.stableId, storageId: config.storageId, parentHandle: nil)
        )
        ...
        let accepted = await crawl(
            impl,
            request: CrawlTriggerRequest(deviceId: config.stableId, storageId: config.storageId, parentHandle: config.objectHandle)
        )
        ...
        let connectedStatus = await status(impl, request: DeviceStatusRequest(deviceId: config.ephemeralId.raw))
        XCTAssertTrue(connectedStatus.connected)
        XCTAssertTrue(connectedStatus.sessionOpen)

        let response = await read(impl, request: ReadRequest(deviceId: config.stableId, objectHandle: config.objectHandle))
    ...
    func testListenerLifecycleAndConnectionAcceptance() async {
        ...
        let accepted = listener.listener(NSXPCListener.anonymous(), shouldAcceptNewConnection: connection)
