...
final class XPCProtocolCoverageTests: XCTestCase {
  ...
  func testReadResponseRoundTripWithAndWithoutSize() throws {
    ...
    let withoutSize = ReadResponse(
      success: false, errorMessage: "boom", tempFileURL: nil, fileSize: nil)
  ...
  func testStorageAndObjectProtocolModelsRoundTrip() throws {
    let storageInfo = StorageInfo(
      storageId: 7, description: "Internal", capacityBytes: 1000, freeBytes: 500)
    ...
    let objectInfo = ObjectInfo(
      handle: 10, name: "photo.jpg", sizeBytes: 4096, isDirectory: false,
    ...
    let objectResponseNoParent = ObjectListRequest(
      deviceId: "domain-3", storageId: 1, parentHandle: nil)
    ...
    let decodedObjectRequestNoParent = try roundTrip(
      objectResponseNoParent, as: ObjectListRequest.self)
  ...
  func testCrawlAndDeviceStatusModelsRoundTrip() throws {
    ...
    let crawlWithoutParent = CrawlTriggerRequest(
      deviceId: "domain-4", storageId: 9, parentHandle: nil)
    ...
    let statusResponse = DeviceStatusResponse(
      connected: true, sessionOpen: true, lastCrawlTimestamp: 123456)
  ...
  func testWriteRequestRoundTrip() throws {
    let original = WriteRequest(
      deviceId: "device1", storageId: 42, parentHandle: 100,
      name: "photo.jpg", size: 1024 * 1024, bookmark: Data([0xDE, 0xAD]))
    ...
    XCTAssertEqual(decoded.deviceId, "device1")
    XCTAssertEqual(decoded.storageId, 42)
    XCTAssertEqual(decoded.parentHandle, 100)
    XCTAssertEqual(decoded.name, "photo.jpg")
    XCTAssertEqual(decoded.size, 1024 * 1024)
    XCTAssertEqual(decoded.bookmark, Data([0xDE, 0xAD]))
  }

  func testWriteRequestRoundTripNilFields() throws {
    let original = WriteRequest(
      deviceId: "dev", storageId: 1, parentHandle: nil,
      name: "root.bin", size: 0, bookmark: nil)
    let data = try NSKeyedArchiver.archivedData(
      withRootObject: original, requiringSecureCoding: true)
    let decoded = try NSKeyedUnarchiver.unarchivedObject(ofClass: WriteRequest.self, from: data)!
  ...
  func testDeleteRequestRoundTrip() throws {
    let original = DeleteRequest(deviceId: "device2", objectHandle: 9999, recursive: true)
    let data = try NSKeyedArchiver.archivedData(
      withRootObject: original, requiringSecureCoding: true)
    let decoded = try NSKeyedUnarchiver.unarchivedObject(ofClass: DeleteRequest.self, from: data)!
    XCTAssertEqual(decoded.deviceId, "device2")
    XCTAssertEqual(decoded.objectHandle, 9999)
    XCTAssertTrue(decoded.recursive)
  }

  func testCreateFolderRequestRoundTrip() throws {
    let original = CreateFolderRequest(
      deviceId: "device3", storageId: 7, parentHandle: 55, name: "NewFolder")
    let data = try NSKeyedArchiver.archivedData(
      withRootObject: original, requiringSecureCoding: true)
    let decoded = try NSKeyedUnarchiver.unarchivedObject(
      ofClass: CreateFolderRequest.self, from: data)!
    XCTAssertEqual(decoded.deviceId, "device3")
    XCTAssertEqual(decoded.storageId, 7)
    XCTAssertEqual(decoded.parentHandle, 55)
    XCTAssertEqual(decoded.name, "NewFolder")
  }

  func testCreateFolderRequestNilParent() throws {
    let original = CreateFolderRequest(
      deviceId: "device3", storageId: 7, parentHandle: nil, name: "TopLevel")
    let data = try NSKeyedArchiver.archivedData(
      withRootObject: original, requiringSecureCoding: true)
    let decoded = try NSKeyedUnarchiver.unarchivedObject(
      ofClass: CreateFolderRequest.self, from: data)!
  ...
  func testWriteResponseRoundTrip() throws {
    let success = WriteResponse(success: true, newHandle: 88)
    let data = try NSKeyedArchiver.archivedData(
    ...
    XCTAssertEqual(decoded.newHandle, 88)
    XCTAssertNil(decoded.errorMessage)
  }

  func testWriteResponseFailureRoundTrip() throws {
    let failure = WriteResponse(success: false, errorMessage: "Device busy", newHandle: nil)
    let data = try NSKeyedArchiver.archivedData(
      withRootObject: failure, requiringSecureCoding: true)
    let decoded = try NSKeyedUnarchiver.unarchivedObject(ofClass: WriteResponse.self, from: data)!
