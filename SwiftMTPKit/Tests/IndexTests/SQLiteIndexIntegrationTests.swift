...
struct SQLiteIndexConcurrencyTests {
  ...
  func testConcurrentWrites() async throws {
    let tempDir = FileManager.default.temporaryDirectory
    let dbPath = tempDir.appendingPathComponent("test-index-concurrent-\(UUID().uuidString).sqlite")
      .path
  ...
  func testNestedTransactionCommit() async throws {
    let tempDir = FileManager.default.temporaryDirectory
    let dbPath = tempDir.appendingPathComponent("test-nested-commit-\(UUID().uuidString).sqlite")
      .path
  ...
  func testNestedTransactionRollback() async throws {
    let tempDir = FileManager.default.temporaryDirectory
    let dbPath = tempDir.appendingPathComponent("test-nested-rollback-\(UUID().uuidString).sqlite")
      .path
