...
private actor MockIndexStore: LiveIndexWriter, LiveIndexReader {
  ...
  private func crawlKey(deviceId: String, storageId: UInt32, parentHandle: MTPObjectHandle?)
    -> String
  {
  ...
  func setCrawlState(
    deviceId: String, storageId: UInt32, parentHandle: MTPObjectHandle?, lastCrawledAt: Date
  ) {
    crawlStateByKey[
      crawlKey(deviceId: deviceId, storageId: storageId, parentHandle: parentHandle)] =
      lastCrawledAt
  }
  ...
  func markStaleChildren(deviceId: String, storageId: UInt32, parentHandle: MTPObjectHandle?)
    async throws
  {
    markStaleCalls += 1
    setCrawlState(
      deviceId: deviceId, storageId: storageId, parentHandle: parentHandle, lastCrawledAt: Date())
  }
...
private actor MockSchedulerDevice: MTPDevice {
  ...
  init(
    ...
    self.summary = MTPDeviceSummary(
      id: self.id, manufacturer: "Mock", model: "SchedulerDevice", vendorID: 0x1234,
      productID: 0x5678)
  ...
  nonisolated func list(parent: MTPObjectHandle?, in storage: MTPStorageID) -> AsyncThrowingStream<
    [MTPObjectInfo], Error
  > {
  ...
  func devGetObjectInfoUncached(handle: MTPObjectHandle) async throws -> MTPObjectInfo {
    try await getInfo(handle: handle)
  }
...
final class IndexCoverageTests: XCTestCase {
  ...
  private func makeDevice(supportsEvents: Bool = true, readDelay: Duration = .zero)
    -> MockSchedulerDevice
  {
  ...
  func testContentCacheLookupMaterializeEvictionPartialAndDownloading() async throws {
    ...
    let firstURL = try await cache.materialize(
      deviceId: "dev", storageId: 1, handle: 11, device: device)
  ...
  func testCrawlSchedulerCoversSeedBoostCrawlAndHandlers() async throws {
    ...
    await scheduler.startPeriodicRefresh(
      deviceId: "dev", device: device, interval: .milliseconds(20))
  ...
  func testDeviceIndexOrchestratorCoversLifecycleAndMaterialization() async throws {
    ...
    let orchestrator = DeviceIndexOrchestrator(
      deviceId: "orchestrator-device", liveIndex: index, contentCache: cache)
    let recorder = ChangeRecorder()
