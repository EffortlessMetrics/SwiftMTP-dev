...
struct IndexOrchestratorTests {
  ...
  func testFullSyncLifecycle() async throws {
    ...
    let dbPath = tempDir.appendingPathComponent("test-orchestrator-\(UUID().uuidString).sqlite")
      .path
  ...
  func testDifferentialUpdates() async throws {
    ...
    let modified = IndexedObject.testObject(
      handle: 0x20001, name: "file1-modified.txt", sizeBytes: 2048)
  ...
  func testSubtreeBoosting() async throws {
    ...
    try await index.markStaleChildren(
      deviceId: "test-device", storageId: 0x10001, parentHandle: 0x20005)
  ...
  func testCrawlStateTracking() async throws {
    ...
    let initialState = try await index.crawlState(
      deviceId: "test-device", storageId: 0x10001, parentHandle: nil)
    ...
    let postCrawlState = try await index.crawlState(
      deviceId: "test-device", storageId: 0x10001, parentHandle: 0x10001)
