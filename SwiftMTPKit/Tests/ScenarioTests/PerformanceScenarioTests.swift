...
struct PerformanceScenarioTests {
  ...
  func testEnumeratePerformanceFastPath() async throws {
    ...
    if objectCount >= 1000 {  // Only check performance for substantial object counts
      ...
      if duration <= maxDuration {
        ...
        Issue.record(
          "Enumeration performance unacceptable: \(String(format: "%.2f", duration))s for \(objectCount) objects"
        )
      }
    }
  }
  ...
  func testCancelLatencyScenario() async throws {
    ...
    let tempURL = URL(fileURLWithPath: NSTemporaryDirectory())
      .appendingPathComponent("cancel-test.tmp")
  ...
  func testResumeReadScenario() async throws {
    ...
    // Find a test file
    guard let testObject = mockData.objects.first(where: { ($0.size ?? 0) > 10 * 1_024 * 1_024 })
    else {
    ...
    print(
      "📊 Testing resume from \(String(format: "%.1f", Double(resumePoint) / Double(fileSize) * 100))%"
    )

    // First, simulate partial transfer
    let partialURL = URL(fileURLWithPath: NSTemporaryDirectory())
      .appendingPathComponent("resume-partial.tmp")
    let finalURL = URL(fileURLWithPath: NSTemporaryDirectory())
      .appendingPathComponent("resume-final.tmp")
    ...
    if FileManager.default.fileExists(atPath: partialURL.path) {
      let partialSize =
        try FileManager.default.attributesOfItem(atPath: partialURL.path)[.size] as? UInt64 ?? 0
      ...
      let resumeProgress = try await device.read(
        handle: testObject.handle, range: resumeRange, to: finalURL)
      ...
      let finalSize =
        try FileManager.default.attributesOfItem(atPath: finalURL.path)[.size] as? UInt64 ?? 0
  ...
  func testDetachMidTransferScenario() async throws {
    ...
    let tempURL = URL(fileURLWithPath: NSTemporaryDirectory())
      .appendingPathComponent("detach-test.tmp")
    defer { try? FileManager.default.removeItem(at: tempURL) }
  ...
  func testIndexIdempotenceScenario() async throws {
    ...
    #expect(
      diff.added.isEmpty && diff.removed.isEmpty && diff.modified.isEmpty,
