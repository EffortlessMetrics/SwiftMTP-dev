...
final class MirrorEngineTests: XCTestCase {
    ...
    func testPathKeyToLocalURL() {
        let localURL = mirrorEngine.pathKeyToLocalURL("0x00010001/DCIM/Camera/photo.jpg", root: tempDirectory)
        XCTAssertTrue(localURL.path.hasSuffix("/DCIM/Camera/photo.jpg"))
    }

    func testGlobPatternMatching() {
        XCTAssertTrue(mirrorEngine.matchesPattern("0x00010001/DCIM/Camera/photo.jpg", pattern: "DCIM/**"))
        XCTAssertTrue(mirrorEngine.matchesPattern("0x00010001/DCIM/Camera/photo.jpg", pattern: "DCIM/*/*.jpg"))
