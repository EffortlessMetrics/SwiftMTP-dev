...
final class MTPObjectEntityPropertyTests: XCTestCase {
  ...
  func testObjectHandleIsAlwaysPositive() {
    property("Object handle should always be positive")
      <- forAll(MTPHandleGenerator.arbitrary) { handle in
        handle > 0
      }
  }

  func testObjectHandleWithinValidRange() {
    property("Object handle should be within valid MTP range (1 to UInt32.max)")
      <- forAll(MTPHandleGenerator.arbitrary) { handle in
  ...
  func testCompoundIdFormatIsValid() {
    property("Compound ID should be in format deviceId:storageId:handle")
      <- forAll(
  ...
  func testParentChildFormsValidTree() {
    property("Tree traversal via parent handles should eventually reach root")
      <- forAll(
        ...
        while depth < maxIterations {
          ...
          guard let index = handles.firstIndex(of: current), index > 0 else {
            break
          }
  ...
  func testParentChainReachesRoot() {
    property("Following parent handles should eventually reach nil (root)")
      <- forAll(
        MTPHandleGenerator.arbitrary
      ) { rootHandle in
  ...
  func testPathReconstructionFromParentChain() {
    property("Path reconstructed from parent chain should match original")
      <- forAll(
        Gen<[String]>
          .fromElements(of: [
            ["DCIM", "2024", "photos"],
  ...
  func testPathKeyRoundtripPreservesData() {
    property("PathKey encode/decode roundtrip should preserve path")
      <- forAll(
  ...
  func testMTPObjectEntityFieldConsistency() {
    property("MTPObjectEntity fields should be consistent after init")
      <- forAll(
        ...
        return entity.deviceId == deviceId && entity.handle == Int(handle)
          && entity.storageId == storageId && entity.name == name
      }
  }
}
...
final class MTPDevicePropertyTests: XCTestCase {
  ...
  func testDeviceIDUniqueness() {
    property("Different device IDs should be different")
      <- forAll(
  ...
  func testDeviceIDSameValueEquality() {
    property("Same device ID values should be equal")
      <- forAll(
  ...
  func testVendorProductIDFormatValidation() {
    property("Vendor/Product ID fingerprint should be in hex format")
      <- forAll(
        ...
        return parts.count == 2 && parts[0].count == 4 && parts[1].count == 4
      }
  }

  func testFingerprintConsistency() {
    property("Fingerprint should be consistent across calls")
      <- forAll(
        Gen<UInt16>.choose((0x0001, 0xFFFF)),
        Gen<UInt16>.choose((0x0001, 0xFFFF))
      ) { vid, pid in
  ...
  func testUnknownFingerprintWhenVIDMissing() {
    property("Fingerprint should be 'unknown' when vendor ID is nil")
      <- forAll(
  ...
  func testValidConnectionStateTransitions() {
    ...
    property("Connection states should transition in valid order")
      <- forAll(
        Gen<Int>.choose((0, 2))
      ) { idx in
  ...
  func testNoSelfTransitionFromConnected() {
    property("Connected state should not transition to itself")
      <- forAll(
        Gen<Int>.pure(0)
      ) { _ -> Bool in
        let validTransitions: [DeviceConnectionState: Set<DeviceConnectionState>] = [
...
final class QuirkDatabasePropertyTests: XCTestCase {
  ...
  func testAddingQuirkDoesNotBreakExistingMatches() {
    property("Adding new quirks should not affect existing matches")
      <- forAll(
        Gen<UInt16>.choose((0x0001, 0xFFFF)),
        Gen<UInt16>.choose((0x0001, 0xFFFF))
      ) { vid, pid in
  ...
  func testFingerprintMatchingIsMonotonic() {
    property("More specific fingerprint matches should score higher")
      <- forAll(
        Gen<UInt16>.choose((0x0001, 0xFFFF)),
        Gen<UInt16>.choose((0x0001, 0xFFFF)),
        ...
        let moreMatch =
          moreSpecific.vid == vid && moreSpecific.pid == pid && moreSpecific.bcdDevice == bcdDevice
  ...
  func testLayerMergingIsAssociative() {
    property("Layer merging should be associative")
      <- forAll(
        Gen<Int>.choose((1, 100)),
        Gen<Int>.choose((1, 100)),
        Gen<Int>.choose((1, 100))
  ...
  func testLayerMergingIsCommutative() {
    property("Layer merging should be commutative")
      <- forAll(
        Gen<Int>.choose((1, 100)),
        Gen<Int>.choose((1, 100))
      ) { a, b in
...
final class TransferJournalPropertyTests: XCTestCase {
  ...
  func testJournalReplayFromAnyPointProducesSameState() {
    property("Journal replay from any point should produce consistent state")
      <- forAll(
        Gen<String>.fromElements(of: ["device1", "device2"]),
  ...
  func testCompactionDoesNotLosePendingTransfers() {
    property("Compaction should preserve pending transfers")
      <- forAll(
        Gen<Int>.choose((1, 20))
      ) { pendingCount in
  ...
  func testCrossDeviceIsolationIsMaintained() {
    property("Transfers from different devices should be isolated")
      <- forAll(
  ...
  func testSameTransferOnDifferentDevicesAreDistinct() {
    property("Same transfer on different devices should be distinct records")
      <- forAll(
...
final class PathKeyPropertyTests: XCTestCase {
  ...
  func testPathKeyRoundtripEncodeDecode() {
    property("PathKey normalize/parse roundtrip should preserve path")
      <- forAll(
        Gen<UInt32>.choose((1, UInt32.max)),
        PathComponentsGenerator.arbitrary
      ) { storageId, components in
  ...
  func testUnicodePathRoundtrip() {
    property("Unicode paths should roundtrip correctly")
      <- forAll(
        Gen<UInt32>.choose((1, UInt32.max)),
        Gen<[String]>
          .fromElements(of: [
  ...
  func testComponentNormalizationIsIdempotent() {
    property("Component normalization should be idempotent")
      <- forAll(
        UnicodeStringGenerator.arbitrary
      ) { component in
  ...
  func testParentResolutionIsCorrect() {
    property("Parent resolution should be correct for all paths")
      <- forAll(
        Gen<UInt32>.choose((1, UInt32.max)),
        Gen<Int>.choose((1, 50))
          .map { depth in
  ...
  func testParentOfRootIsNil() {
    property("Parent of root path should be nil")
      <- forAll(
        Gen<UInt32>.choose((1, UInt32.max))
      ) { storageId in
  ...
  func testGrandparentChainReachesRoot() {
    property("Following parent chain should eventually reach root")
      <- forAll(
        Gen<UInt32>.choose((1, UInt32.max)),
        Gen<Int>.choose((1, 20))
      ) { storageId, depth in
...
final class ThroughputMetricsPropertyTests: XCTestCase {
  ...
  func testMovingAverageIsWithinReasonableBounds() {
    property("EWMA should stay within reasonable bounds")
      <- forAll(
  ...
  func testEWMAResetClearsState() {
    property("EWMA reset should clear all state")
      <- forAll(
  ...
  func testEWMAPositiveUpdateNeverDecreases() {
    property("EWMA with consistent samples should not decrease")
      <- forAll(
        Gen<Int>.choose((1000, 1_000_000)),
        Gen<Double>.choose((0.01, 10.0)),
        Gen<Int>.choose((2, 20))
  ...
  func testRingBufferCapacity() {
    property("Ring buffer should respect max capacity")
      <- forAll(
  ...
  func testRingBufferWrapsCorrectly() {
    property("Ring buffer should wrap around correctly")
      <- forAll(
        Gen<Int>.choose((5, 20)),
  ...
  func testRingBufferAverageCalculation() {
    property("Ring buffer average should be correct")
      <- forAll(
        Gen<Int>.choose((5, 20))
      ) { count in
  ...
  func testRegressionDetectionThresholdIsStable() {
    property("Regression threshold should be stable across calculations")
      <- forAll(
        Gen<Int>.choose((10, 100))
      ) { count in
        var buffer = ThroughputRingBuffer(maxSamples: count)
  ...
  func testThroughputRateNeverNegative() {
    property("Throughput rate should never be negative")
      <- forAll(
...
final class EdgeCasePropertyTests: XCTestCase {
  ...
  func testEmptyComponentNormalization() {
    property("Empty components should normalize to underscore")
      <- forAll(
        Gen<Int>.pure(0)
      ) { _ -> Bool in
  ...
  func testControlCharacterStripping() {
    property("Control characters should be stripped from paths")
      <- forAll(
        Gen<String>
          .fromElements(of: [
            "file\u{00}.txt",
  ...
  func testZeroHandleIsInvalid() {
    property("Zero handle should be treated as invalid")
      <- forAll(
        Gen<Int>.pure(0)
      ) { _ -> Bool in
  ...
  func testZeroStorageIdHandling() {
    property("Zero storage ID should be handled correctly")
      <- forAll(
        Gen<[String]>
          .fromElements(of: [
            ["test"],
  ...
  func testMaximumHandleValue() {
    property("Maximum handle value should be valid")
      <- forAll(
        Gen<Int>.pure(0)
      ) { _ -> Bool in
  ...
  func testLargeFileSizeHandling() {
    property("Large file sizes should be handled correctly")
      <- forAll(
  ...
  func testEmojiHandling() {
    property("Emoji should be handled (potentially stripped)")
      <- forAll(
        Gen<String>
          .fromElements(of: [
            "🎉",
  ...
  func testMixedUnicodeNormalization() {
    property("Mixed Unicode should normalize correctly")
      <- forAll(
        Gen<String>
          .fromElements(of: [
            "café-🎉-файл-文件.txt",
