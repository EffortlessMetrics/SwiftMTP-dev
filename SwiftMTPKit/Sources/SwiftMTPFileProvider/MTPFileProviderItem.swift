...
public final class MTPFileProviderItem: NSObject, NSFileProviderItem {
  ...
  public init(
    deviceId: String, storageId: UInt32?, objectHandle: UInt32?, parentHandle: UInt32? = nil,
    name: String, size: UInt64?, isDirectory: Bool, modifiedDate: Date?
  ) {
  ...
  public var parentItemIdentifier: NSFileProviderItemIdentifier {
    if let _ = objectHandle, let sid = storageId {
      guard let ph = _parentHandle else {
  ...
  public struct ItemComponents: Sendable {
    public let deviceId: String
    public let storageId: UInt32?
    public let objectHandle: UInt32?
  }

  public static func parseItemIdentifier(_ identifier: NSFileProviderItemIdentifier)
    -> ItemComponents?
  {
    ...
    if parts.count == 1 {
      ...
      return ItemComponents(
        deviceId: String(parts[0]), storageId: UInt32(parts[1]), objectHandle: nil)
    } else if parts.count == 3 {
      return ItemComponents(
        deviceId: String(parts[0]), storageId: UInt32(parts[1]), objectHandle: UInt32(parts[2]))
    }
    return nil
  }
}
