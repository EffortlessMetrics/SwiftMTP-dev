...
public final class MTPFileProviderManager: Sendable {
    ...
    public func registerDomain(identity: StableDeviceIdentity) async throws {
        ...
        do {
            ...
            log.info("Registered FileProvider domain for \(identity.displayName) (domainId: \(identity.domainId))")
        } catch {
            log.error("Failed to register FileProvider domain for \(identity.displayName): \(error.localizedDescription)")
            throw error
        }
    }
    ...
    public func unregisterDomain(domainId: String) async throws {
        ...
        do {
            ...
            log.error("Failed to unregister FileProvider domain \(domainId): \(error.localizedDescription)")
            throw error
        }
    }
