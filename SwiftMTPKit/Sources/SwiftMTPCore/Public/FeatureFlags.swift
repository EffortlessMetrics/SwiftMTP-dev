  ...
  private init() {
    ...
    for (key, value) in ProcessInfo.processInfo.environment {
      if key.starts(with: "SWIFTMTP_") {
        flags.withLock { $0[key] = (value == "1" || value.lowercased() == "true") }
      }
    }

    // Load from arguments (useful for XCTest / CLI flags like --demo-mode)
    let args = ProcessInfo.processInfo.arguments
    if args.contains("--demo-mode") { flags.withLock { $0["SWIFTMTP_DEMO_MODE"] = true } }
    if args.contains("--storybook") { flags.withLock { $0["SWIFTMTP_SHOW_STORYBOOK"] = true } }
    if args.contains("--trace-usb") { flags.withLock { $0["SWIFTMTP_TRACE_USB"] = true } }
  }
