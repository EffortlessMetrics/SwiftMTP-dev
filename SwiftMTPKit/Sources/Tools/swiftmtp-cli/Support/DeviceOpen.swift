...
func openDevice(flags: CLIFlags) async throws -> any MTPDevice {
  ...
  if flags.useMock {
    ...
    return try await manager.openDevice(
      with: mock, transport: LibUSBTransportFactory.createTransport())
  }
  ...
  return try await manager.openDevice(
    with: selectedDevice, transport: LibUSBTransportFactory.createTransport(), config: config)
}
...
private func logDeviceCandidates(_ devices: [MTPDeviceSummary]) {
  ...
  for (index, device) in devices.enumerated() {
    ...
    log(
      "  [\(index + 1)] \(device.manufacturer) \(device.model)  vid:pid=\(vid):\(pid)  bus:addr=\(busAddr)"
    )
  }
}
