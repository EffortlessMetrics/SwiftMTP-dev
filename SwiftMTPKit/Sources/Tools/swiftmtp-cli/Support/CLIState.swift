...
public func parseSize(_ str: String) -> UInt64 {
  ...
  if let mult = suffix?.uppercased().first, let multiplier = multipliers[mult] {
...
public func printJSONErrorAndExit(
  _ message: String, code: ExitCode, details: [String: String]? = nil
) -> Never {
  ...
  if let data = try? JSONSerialization.data(withJSONObject: errorDict, options: [.sortedKeys]) {
    ...
    fputs(
      "{\"type\":\"error\",\"schemaVersion\":\"1.0\",\"message\":\"encoding failed\"}\n", stderr)
  }
  exitNow(code)
}
