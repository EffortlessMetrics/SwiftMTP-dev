...
@main struct TestXiaomi {
  static func main() async {
    ...
    do {
      ...
      guard rc == 0, let ctx = ctx else {
        throw NSError(
          domain: "LibUSB", code: Int(rc),
          userInfo: [NSLocalizedDescriptionKey: "Failed to initialize libusb"])
      }
      ...
      guard cnt > 0, let list = list else {
        throw NSError(
          domain: "LibUSB", code: -1, userInfo: [NSLocalizedDescriptionKey: "No USB devices found"])
      }
      ...
      guard foundDevice else {
        throw NSError(
          domain: "Device", code: -1,
          userInfo: [NSLocalizedDescriptionKey: "Xiaomi Mi Note 2 not found"])
      }
      ...
      let mtpDevice = try await MTPDeviceManager.shared.openDevice(
        with: deviceSummary, transport: transport, config: config)
      ...
      if let firstStorage = storages.first {
        ...
        let objects = await listObjects(
          device: mtpDevice, storage: firstStorage.id, parent: nil, maxCount: 10)
  ...
  static func listObjects(
    device: any MTPDevice, storage: MTPStorageID, parent: MTPObjectHandle?, maxCount: Int
  ) async -> [MTPObjectInfo] {
    do {
