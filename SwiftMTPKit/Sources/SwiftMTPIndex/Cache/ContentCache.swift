...
public actor ContentCache {
    ...
    public func lookup(deviceId: String, storageId: UInt32, handle: MTPObjectHandle) -> CacheResult {
        ...
        do {
            let sql = "SELECT localPath, state, committedBytes FROM cached_content WHERE deviceId = ? AND storageId = ? AND handle = ?"
    ...
    public func evictIfNeeded() throws {
        ...
        let lruSQL = "SELECT deviceId, storageId, handle, localPath, sizeBytes FROM cached_content WHERE state = 'complete' ORDER BY lastAccessedAt ASC"
        try db.withStatement(lruSQL) { stmt in
            while toFree > 0, try db.step(stmt) {
                ...
                // Delete DB record
                let delSQL = "DELETE FROM cached_content WHERE deviceId = ? AND storageId = ? AND handle = ?"
    ...
    private func touchAccessTime(deviceId: String, storageId: UInt32, handle: MTPObjectHandle) throws {
        let now = Int64(Date().timeIntervalSince1970)
        let sql = "UPDATE cached_content SET lastAccessedAt = ? WHERE deviceId = ? AND storageId = ? AND handle = ?"
        try db.withStatement(sql) { stmt in
