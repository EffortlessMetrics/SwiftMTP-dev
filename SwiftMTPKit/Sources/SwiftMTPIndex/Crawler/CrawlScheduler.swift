...
public actor CrawlScheduler {
  ...
  public func seedOnConnect(deviceId: String, device: any MTPDevice) async {
    do {
      ...
      for storage in storages {
        ...
        try await indexWriter.upsertStorage(
          IndexedStorage(
            deviceId: deviceId,
        ...
        enqueueJob(
          deviceId: deviceId, storageId: storage.id.raw, parentHandle: nil, priority: .foreground)
      }
    } catch {
  ...
  public func handleObjectAdded(deviceId: String, handle: MTPObjectHandle, device: any MTPDevice)
    async
  {
    do {
  ...
  public func handleObjectRemoved(deviceId: String, handle: MTPObjectHandle) async {
    ...
    do {
      ...
      if let obj = try await (indexWriter as? LiveIndexReader)?
        .object(deviceId: deviceId, handle: handle)
      {
        try await indexWriter.removeObject(
          deviceId: deviceId, storageId: obj.storageId, handle: handle)
  ...
  public func startPeriodicRefresh(
    deviceId: String, device: any MTPDevice, interval: Duration = .seconds(30)
  ) {
    Task { [weak self] in
  ...
  private func enqueueJob(
    deviceId: String, storageId: UInt32, parentHandle: MTPObjectHandle?, priority: CrawlPriority
  ) {
    let job = CrawlJob(
      id: nextId, deviceId: deviceId, storageId: storageId, parentHandle: parentHandle,
      priority: priority)
    nextId += 1
    queue.append(job)
    queue.sort()
  ...
  private func crawlFolder(job: CrawlJob, device: any MTPDevice) async {
    ...
    do {
      ...
      try await indexWriter.markStaleChildren(
        deviceId: job.deviceId, storageId: job.storageId, parentHandle: job.parentHandle)
      ...
      for try await batch in stream {
        for objInfo in batch {
          ...
          if isDir {
            enqueueJob(
              deviceId: job.deviceId, storageId: job.storageId, parentHandle: objInfo.handle,
              priority: .background)
          }
        }
      }
      ...
      try await indexWriter.purgeStale(
        deviceId: job.deviceId, storageId: job.storageId, parentHandle: job.parentHandle)
