...
public struct MTPDiff: Sendable {
  ...
  public struct Row: Sendable {
    ...
    public init(
      handle: UInt32, storage: UInt32, pathKey: String, size: UInt64?, mtime: Date?, format: UInt16
    ) {
      self.handle = handle
...
public final class DiffEngine: Sendable {
  ...
  public func diff(deviceId: MTPDeviceID, oldGen: Int?, newGen: Int) async throws -> MTPDiff {
    log.info(
      "Computing diff for device \(deviceId.raw) between generations \(oldGen ?? -1) and \(newGen)")
    ...
    log.info(
      "Diff computation completed for device \(deviceId.raw): +\(diff.added.count) -\(diff.removed.count) ~\(diff.modified.count) in \(duration)s"
    )
  ...
  private func loadObjects(deviceId: MTPDeviceID, gen: Int?) async throws -> [String: ObjectRecord]
  {
    ...
    if let persistence = self.persistence {
      let records = try await persistence.objectCatalog.fetchObjects(
        deviceId: deviceId, generation: gen)

      if !records.isEmpty {
        for record in records {
          let row = MTPDiff.Row(
            handle: record.handle, storage: record.storage, pathKey: record.pathKey,
    ...
    try db.withStatement(
      "SELECT storageId, handle, pathKey, size, mtime, format FROM objects WHERE deviceId = ? AND gen = ?"
    ) { stmt in
      ...
      while try db.step(stmt) {
        ...
        let row = MTPDiff.Row(
          handle: handle, storage: storageId, pathKey: pathKey,
