...
public final class Snapshotter: Sendable {
  ...
  public func capture(device: any MTPDevice, deviceId: MTPDeviceID) async throws -> Int {
    ...
    Task {
      ...
      try? await persistence.snapshots.recordSnapshot(
        deviceId: deviceId, generation: gen, path: nil, hash: nil)
    }

    let duration = Date().timeIntervalSince(start)
    log.info(
      "Snapshot capture completed for device \(deviceId.raw), generation \(gen), duration \(duration)s"
    )

    return gen
  }

  private func captureDeviceInfo(device: any MTPDevice, deviceId: MTPDeviceID, timestamp: Date)
    async throws
  {
    let info = try await device.info

    try db.withStatement("INSERT OR REPLACE INTO devices(id, model, lastSeenAt) VALUES(?,?,?)") {
      stmt in
  ...
  private func captureStorageInfo(storage: MTPStorageInfo, deviceId: MTPDeviceID, timestamp: Date)
    async throws
  {
    try db.withStatement(
      "INSERT OR REPLACE INTO storages(id, deviceId, description, capacity, free, readOnly, lastIndexedAt) VALUES(?,?,?,?,?,?,?)"
    ) { stmt in
  ...
  private func captureObjects(
    device: any MTPDevice, storage: MTPStorageInfo, deviceId: MTPDeviceID, gen: Int
  ) async throws {
    ...
    do {
      ...
      log.error(
        "Failed to list objects for storage \(storage.id.raw) on device \(deviceId.raw): \(error.localizedDescription)"
      )
    ...
    for batchStart in stride(from: 0, to: allObjects.count, by: batchSize) {
      ...
      try await processObjectBatch(
        batch, storage: storage, deviceId: deviceId, gen: gen,
        parentMap: parentMap, nameMap: nameMap)
    }
  }

  private func processObjectBatch(
    _ objects: [MTPObjectInfo], storage: MTPStorageInfo, deviceId: MTPDeviceID, gen: Int,
    parentMap: [UInt32: UInt32], nameMap: [UInt32: String]
  ) async throws {
    var batchToRecord: [(object: MTPObjectInfo, pathKey: String)] = []

    for object in objects {
      let pathComponents = buildPathComponents(
        for: object.handle, parentMap: parentMap, nameMap: nameMap)
    ...
    let persistence = await MTPDeviceManager.shared.persistence
    try? await persistence.objectCatalog.recordObjects(
      deviceId: deviceId, objects: batchToRecord, generation: gen)
  }

  internal func buildPathComponents(
    for handle: UInt32, parentMap: [UInt32: UInt32], nameMap: [UInt32: String]
  ) -> [String] {
    ...
    while let name = nameMap[currentHandle], !visited.contains(currentHandle) {
      ...
      guard let parent = parentMap[currentHandle] else {
        break
      }
      ...
      if visited.count > 1000 {
        log.warning(
          "Path building detected potential cycle or excessive depth for handle \(handle), depth \(visited.count)"
        )
        break
      }
    }
  ...
  private func markPreviousGenerationTombstoned(deviceId: MTPDeviceID, currentGen: Int) throws {
    try db.withStatement(
      "UPDATE objects SET tombstone = 1 WHERE deviceId = ? AND gen < ? AND tombstone = 0"
    ) { stmt in
      try db.bind(stmt, 1, deviceId.raw)
  ...
  public func latestGeneration(for deviceId: MTPDeviceID) throws -> Int? {
    let result = try db.withStatement(
      "SELECT gen FROM snapshots WHERE deviceId = ? ORDER BY gen DESC LIMIT 1"
    ) { stmt -> Int64? in
      try db.bind(stmt, 1, deviceId.raw)
  ...
  public func previousGeneration(for deviceId: MTPDeviceID, before currentGen: Int) throws -> Int? {
    let result = try db.withStatement(
      "SELECT gen FROM snapshots WHERE deviceId = ? AND gen < ? ORDER BY gen DESC LIMIT 1"
    ) { stmt -> Int64? in
      try db.bind(stmt, 1, deviceId.raw)
      try db.bind(stmt, 2, Int64(currentGen))
      return try db.step(stmt) ? db.colInt64(stmt, 0) : nil
