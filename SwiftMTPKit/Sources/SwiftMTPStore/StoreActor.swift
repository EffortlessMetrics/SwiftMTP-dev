...
public actor StoreActor {
    ...
    public func fetchResumableTransfers(for deviceId: String) throws -> [TransferRecord] {
        ...
        return entities.map { entity in
            TransferRecord(
                ...
                supportsPartial: entity.supportsPartial, localTempURL: URL(fileURLWithPath: entity.localTempURL),
    ...
    public func upsertObject(
        ...
        if let object = existing {
            ...
            let devicePredicate = #Predicate<DeviceEntity> { $0.id == deviceId }
            object.device = try modelContext.fetch(FetchDescriptor<DeviceEntity>(predicate: devicePredicate)).first
    ...
    public func upsertObjects(
        deviceId: String,
        objects: [(storageId: Int, handle: Int, parentHandle: Int?, name: String, pathKey: String, size: Int64?, mtime: Date?, format: Int, generation: Int)]
    ) throws {
        let devicePredicate = #Predicate<DeviceEntity> { $0.id == deviceId }
        guard let deviceRef = try modelContext.fetch(FetchDescriptor<DeviceEntity>(predicate: devicePredicate)).first else {
            return
        }
    ...
    public func markPreviousGenerationTombstoned(deviceId: String, currentGen: Int) throws {
        let predicate = #Predicate<MTPObjectEntity> { $0.deviceId == deviceId && $0.generation < currentGen }
    ...
    public func fetchObjects(deviceId: String, generation: Int) throws -> [MTPObjectRecord] {
        let predicate = #Predicate<MTPObjectEntity> { $0.deviceId == deviceId && $0.generation == generation && $0.tombstone == 0 }
