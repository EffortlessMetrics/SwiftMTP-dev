...
public actor StoreActor {
  ...
  public func upsertDevice(id: String, manufacturer: String?, model: String?) throws -> String {
    ...
    guard let device = existing.first else {
  ...
  // MARK: - Learned Profiles

  public func updateLearnedProfile(
    for fingerprintHash: String, deviceId: String, profile: LearnedProfile
  ) throws {
    _ = try upsertDevice(id: deviceId, manufacturer: nil, model: nil)

    let devicePredicate = #Predicate<DeviceEntity> { $0.id == deviceId }
    let device = try modelContext.fetch(FetchDescriptor<DeviceEntity>(predicate: devicePredicate))
      .first
  ...
  public func recordProfilingRun(deviceId: String, profile: MTPDeviceProfile) throws {
    _ = try upsertDevice(
      id: deviceId, manufacturer: profile.deviceInfo.manufacturer, model: profile.deviceInfo.model)
  ...
  public func recordSnapshot(deviceId: String, generation: Int, path: String?, hash: String?) throws
  {
    ...
    let devicePredicate = #Predicate<DeviceEntity> { $0.id == deviceId }
    let deviceRef =
      try modelContext.fetch(FetchDescriptor<DeviceEntity>(predicate: devicePredicate)).first
  ...
  public func createTransfer(
    id: String, deviceId: String, kind: String, handle: UInt32?, parentHandle: UInt32?,
    name: String, totalBytes: UInt64?, supportsPartial: Bool, localTempURL: String,
    finalURL: String?, etagSize: UInt64?, etagMtime: Date?
  ) throws {
    let transfer = TransferEntity(
      id: id, deviceId: deviceId, kind: kind, handle: handle, parentHandle: parentHandle,
      name: name, totalBytes: totalBytes, supportsPartial: supportsPartial,
      localTempURL: localTempURL, finalURL: finalURL
    )
  ...
  public func fetchResumableTransfers(for deviceId: String) throws -> [TransferRecord] {
    ...
    let entities = try modelContext.fetch(FetchDescriptor<TransferEntity>(predicate: predicate))

    return entities.map { entity in
      TransferRecord(
        id: entity.id, deviceId: MTPDeviceID(raw: entity.deviceId), kind: entity.kind,
        handle: entity.handle, parentHandle: entity.parentHandle, name: entity.name,
        totalBytes: entity.totalBytes, committedBytes: entity.committedBytes,
  ...
  public func upsertStorage(
    deviceId: String, storageId: Int, description: String,
    capacity: Int64, free: Int64, readOnly: Bool
  ) throws {
    let compoundId = "\(deviceId):\(storageId)"
    let predicate = #Predicate<MTPStorageEntity> { $0.compoundId == compoundId }
    let existing = try modelContext.fetch(FetchDescriptor<MTPStorageEntity>(predicate: predicate))
      .first
    ...
    if let storage = existing {
      ...
      let storage = MTPStorageEntity(
        deviceId: deviceId, storageId: storageId, storageDescription: description,
        capacityBytes: capacity, freeBytes: free, isReadOnly: readOnly
      )
      let devicePredicate = #Predicate<DeviceEntity> { $0.id == deviceId }
      storage.device =
        try modelContext.fetch(FetchDescriptor<DeviceEntity>(predicate: devicePredicate)).first
  ...
  public func upsertObject(
    deviceId: String, storageId: Int, handle: Int, parentHandle: Int?,
    ...
    let existing = try modelContext.fetch(FetchDescriptor<MTPObjectEntity>(predicate: predicate))
      .first
    ...
    if let object = existing {
      ...
      let object = MTPObjectEntity(
        ...
        parentHandle: parentHandle, name: name, pathKey: pathKey,
        sizeBytes: size, modifiedAt: mtime, formatCode: format,
  ...
  public func upsertObjects(
    deviceId: String,
    objects: [(
      storageId: Int, handle: Int, parentHandle: Int?, name: String, pathKey: String, size: Int64?,
      mtime: Date?, format: Int, generation: Int
    )]
  ) throws {
    let devicePredicate = #Predicate<DeviceEntity> { $0.id == deviceId }
    guard
    ...
    for o in objects {
      let compoundId = "\(deviceId):\(o.storageId):\(o.handle)"
      let predicate = #Predicate<MTPObjectEntity> { $0.compoundId == compoundId }
      let existing = try modelContext.fetch(FetchDescriptor<MTPObjectEntity>(predicate: predicate))
        .first

      if let object = existing {
        object.parentHandle = o.parentHandle
        object.name = o.name
        object.pathKey = o.pathKey
        object.sizeBytes = o.size
        object.modifiedAt = o.mtime
        object.formatCode = o.format
        object.generation = o.generation
      } else {
        let object = MTPObjectEntity(
          deviceId: deviceId, storageId: o.storageId, handle: o.handle,
          parentHandle: o.parentHandle, name: o.name, pathKey: o.pathKey,
          sizeBytes: o.size, modifiedAt: o.mtime, formatCode: o.format,
          generation: o.generation
        )
        object.device = deviceRef
        modelContext.insert(object)
      }
    }
    try modelContext.save()
  ...
  public func markPreviousGenerationTombstoned(deviceId: String, currentGen: Int) throws {
    let predicate = #Predicate<MTPObjectEntity> {
      $0.deviceId == deviceId && $0.generation < currentGen
    }
  ...
  public func fetchObjects(deviceId: String, generation: Int) throws -> [MTPObjectRecord] {
    let predicate = #Predicate<MTPObjectEntity> {
      $0.deviceId == deviceId && $0.generation == generation && $0.tombstone == 0
    }
    let entities = try modelContext.fetch(FetchDescriptor<MTPObjectEntity>(predicate: predicate))
