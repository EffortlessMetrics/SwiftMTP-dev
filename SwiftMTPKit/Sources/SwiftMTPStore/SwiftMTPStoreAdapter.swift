...
public final class SwiftMTPStoreAdapter: MTPPersistenceProvider, LearnedProfileStore,
  ProfilingStore, SnapshotStore, SubmissionStore, TransferJournal, ObjectCatalogStore, Sendable
{
  ...
  public func saveProfile(_ profile: LearnedProfile, for deviceId: MTPDeviceID) async throws {
    ...
    try await actor.updateLearnedProfile(
      for: profile.fingerprintHash, deviceId: deviceId.raw, profile: profile)
  }
  ...
  public func recordSnapshot(deviceId: MTPDeviceID, generation: Int, path: String?, hash: String?)
    async throws
  {
    let actor = store.createActor()
    try await actor.recordSnapshot(
      deviceId: deviceId.raw, generation: generation, path: path, hash: hash)
  }
  ...
  public func recordObject(
    deviceId: MTPDeviceID, object: MTPObjectInfo, pathKey: String, generation: Int
  ) async throws {
    let actor = store.createActor()
  ...
  public func recordObjects(
    deviceId: MTPDeviceID, objects: [(object: MTPObjectInfo, pathKey: String)], generation: Int
  ) async throws {
    let actor = store.createActor()
