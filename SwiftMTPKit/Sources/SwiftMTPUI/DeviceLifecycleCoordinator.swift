...
public final class DeviceLifecycleCoordinator {
  ...
  private func bootstrapUITestScenario() async throws {
    ...
    let device = try await manager.openDevice(
      with: summary, transport: transport, config: SwiftMTPConfig())
  ...
  private func handleAttach(summary: MTPDeviceSummary) async {
    ...
    do {
      ...
      self.error =
        "Failed to set up device \(summary.manufacturer) \(summary.model): \(error.localizedDescription)"
    }
  }
